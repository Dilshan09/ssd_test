    ...
    const onSubmit = async (e) => {
      ...
      const handleLogin = (e) => {
        ...
        if (isAuthenticated) {
          ...
          <span className='text-sm text-red-300 font-bold'>Session Over!!</span>
        }
      };
            ...
            <main className="w-full h-screen flex self-center place-content-center place-items-center">
                <div className="w-96 text-white space-y-5 p-4 shadow-xl border rounded-xl">
                    ...
                    >
                        <div>
                            ...
                            <input
                                ...
                                className="w-full mt-2 px-3 py-2 text-white bg-transparent outline-none border focus:border-white shadow-sm rounded-lg transition duration-300"
                            />
                        </div>
                        ...
                        {errorMessage && (
                            <span className='text-sm text-red-300 font-bold'>{errorMessage}</span>
                        )}

                        <button
                            type="submit"
                            disabled={isSigningIn}
                            className={`w-full px-4 py-2 text-white font-medium rounded-lg ${isSigningIn ? 'bg-white-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 hover:shadow-xl transition duration-300'}`}
                        >
                    ...
                    <button
                        ...
                        className={`w-full flex items-center justify-center gap-x-3 py-2.5 border rounded-lg text-sm font-medium  ${isSigningIn ? 'cursor-not-allowed' : 'hover:bg-white-100 transition duration-300 active:bg-white-100'}`}>
